...
def month_plot(var):
    ...
    months=pd.DataFrame({
        'Mont': np.arange(1,13,1),#["Jan","Feb","Mär","April","Mai",],
        'RMSE_temp_multi':rmses_month(lstm_multi,var),
        'RMSE_temp_uni':rmses_month(lstm_uni,var),
        'skills':[(1-(x/y)) for x,y in zip(rmses_month(lstm_multi,var),referencess)],
        'skills_uni':[(1-(x/y)) for x,y in zip(rmses_month(lstm_uni,var),referencess)],
    'skills_nhit':[(1-(x/y)) for x,y in zip(rmses_month(nhits,var),referencess)]


    })

    #print( math.sqrt(mean_squared_error(data["temp"], model["temp"])))df.loc[(df['date'].month==12)

    plt.plot(months["skills"])
    plt.plot(months["skills_uni"])
    plt.plot(months["skills_nhit"])
